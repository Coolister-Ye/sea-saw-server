    ...
    def clean(self):
        ...
        if self.field_type == FieldType.PICKLIST and not self.extra_info:
            raise ValidationError(
                {"extra_info": _("Picklist fields must have extra information (e.g., picklist choices).")}
            )
...
class Contact(BaseModel):
    ...
    def __str__(self):
        ...
        联系人的字符串表示，组合名和姓。
        """
        return f"{self.first_name or ''} {self.last_name or ''}".strip()  # 返回完整姓名 (Return Full Name)
...
class Contract(BaseModel):
    ...
    class Meta:
        ...
        ordering = ['-created_at']  # 按创建时间倒序排列 (Order by Creation Time Descending)
...
class Order(BaseModel):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['order_code']),  # 创建订单编号索引 (Index by Order Code)
