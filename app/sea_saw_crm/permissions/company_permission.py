...
专门针对 Company 模型的权限控制。
Company 允许 Admin 和 Sale 角色访问，基于 owner 进行访问控制。
"""
...
class CompanySalePermission(BasePermission):
    """
    Company Sale 权限

    Sale 角色基于 owner 的 Company 权限：
    - 读取权限：可以查看自己或可见用户创建的公司
    - 创建权限：可以创建新公司
    - 修改/删除权限：只能修改/删除自己创建的公司
    """
