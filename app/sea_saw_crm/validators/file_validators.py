...
DANGEROUS_EXTENSIONS = {
    '.exe', '.dll', '.bat', '.cmd', '.sh', '.ps1', '.psm1',
    '.msi', '.app', '.deb', '.rpm', '.dmg', '.pkg',
    '.scr', '.com', '.pif', '.application', '.gadget',
    '.msp', '.jar', '.js', '.jse', '.vbs', '.vbe',
    '.ws', '.wsf', '.wsh', '.hta', '.cpl', '.msc',
    '.reg', '.scf', '.lnk', '.inf', '.vb',
}
...
def validate_file_mime_type(value):
    ...
    if HAS_MAGIC:
        ...
        if not detected_type:
            detected_type = declared_type
            logger.warning(f"Could not detect MIME type, using declared type: {declared_type}")
    ...
    if ext not in allowed_extensions:
        raise ValidationError(
            _('File extension "%(ext)s" does not match detected file type "%(type)s". '
